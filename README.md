...
*   **Sleep Prevention**: Automatically prevents the system from entering sleep or idle modes during active backup, restore, or prune operations. This ensures that long-running tasks are not interrupted by power management settings. Supports Windows, macOS, and Linux (via `systemd-inhibit`).
...
shasum -a 256 pgl-backup_v1.4.5_darwin_arm64.tar.gz
# Compare output with the content of checksums.txt
```

**Windows (PowerShell):**
```powershell
Get-FileHash .\pgl-backup_v1.4.5_windows_amd64.zip -Algorithm SHA256
