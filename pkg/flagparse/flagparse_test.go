...
func TestParseFlagConfig(t *testing.T) {
	t.Run("No Flags - Returns Zero-Value Config", func(t *testing.T) {
		runTestWithFlags(t, []string{}, func() {
			...
			if act != BackupAction {
				t.Errorf("expected action to be BackupAction, but got %v", act)
			}
			if len(setFlags) != 0 {
	...
	t.Run("Set Action Flags", func(t *testing.T) {
		testCases := []struct {
			...
			{"Version Flag", "-version", VersionAction},
			{"Init Flag", "-init", InitAction},
			{"Init Default Flag", "-init-default", InitDefaultAction},
		}
	...
	t.Run("Conflicting Flags", func(t *testing.T) {
		args := []string{"-init", "-init-default"}
		runTestWithFlags(t, args, func() {
			_, _, err := ParseFlagConfig()
			if err == nil {
				t.Fatal("expected error for conflicting flags, but got nil")
			}
		})
	})

	t.Run("Three Conflicting Flags", func(t *testing.T) {
		args := []string{"-init", "-version", "-backup"}
		runTestWithFlags(t, args, func() {
			_, _, err := ParseFlagConfig()
			if err == nil {
				t.Fatal("expected error for conflicting flags, but got nil")
