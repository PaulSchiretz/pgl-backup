...
func (r *PathRetentionManager) Apply(ctx context.Context, retentionPolicyTitle string, dirPath string, retentionPolicy config.RetentionPolicyConfig, excludeDir string) error {
	...
	if len(runState.backups) == 0 {
		plog.Debug(fmt.Sprintf("No %s backups found to apply retention to", runState.retentionPolicyTitle))
		return nil
	}
	...
	for i := 0; i < numWorkers; i++ {
		...
		go func(workerID int) {
			...
			for b := range deleteDirTasksChan {
				...
				if r.config.DryRun {
					plog.Info("[DRY RUN] Would delete directory", "policy", runState.retentionPolicyTitle, "path", dirToDelete)
					continue
				}
